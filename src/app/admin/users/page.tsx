    ...
    return (
      <div key={user.id} className="flex flex-col items-center">
        <div className="relative p-2 flex flex-col items-center group">
          ...
          {user.manager_id && (
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-px h-2 bg-primary/20" />
          )}
          
          <Card className="w-40 border-2 border-primary/10 hover:border-primary/40 transition-all hover:shadow-lg bg-white z-10 scale-[0.95]">
            <CardContent className="p-3 flex flex-col items-center gap-2">
              <div className="relative">
                <Avatar className="h-12 w-12 border-4 border-white shadow-md">
                  <AvatarImage src={`https://picsum.photos/seed/${user.id}/48/48`} />
              ...
              <div className="text-center overflow-hidden w-full">
                <div className="font-black text-xs tracking-tight truncate px-1">{user.name}</div>
                <div className="text-[8px] text-muted-foreground uppercase font-black tracking-widest mt-0.5">{user.role}</div>
              </div>
              {hasReports && (
                <Badge variant="secondary" className="text-[7px] h-3 px-1.5 font-bold">
      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <header className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            ...
            <p className="text-muted-foreground mt-2 text-sm max-w-lg">Visualizing hierarchy and authority for <span className="font-black text-primary border-b-2 border-primary/20">{company.name}</span>.</p>
          </div>
            ...
            {viewType === 'LIST' ? (
              ...
              <div className="overflow-auto p-8 bg-[#f8fafc] animate-in" style={{ height: 'calc(100vh - 350px)' }}>
                <div className="inline-flex flex-col items-center min-w-full origin-top transform transition-transform duration-300">
                  {rootUsers.map(user => (
                    <div key={user.id} className="mb-10 last:mb-0">
